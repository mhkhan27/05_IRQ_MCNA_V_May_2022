...
# saveWidget(base_map, "s.html",title = "Returnees sample point")


# read_dataset ------------------------------------------------------------

returnees_sample_frame <- read.csv("03_outputs/01_sampling/02_sampling/03_Returnees/sampling_frame.csv")
returnees_sample_frame$district %>% unique()
sample_frame_sf <- returnees_sample_frame %>% st_as_sf(coords = c("longitude","latitude"),crs = 4326)
...
# livable_zone <- st_read("01_inputs/03_shapefile/cluster_for_retunees_sampling/cluster_dis.shp")
# livable_zone <- livable_zone %>% st_transform(crs= 32638)
...
sample_bffer_final$location_name_in_english <-  sample_bffer_final$location_name_in_english   %>% trimws() %>% str_replace_all(" / ","_")
...
sample_points$local_and_id <- sample_points$local_and_id %>% trimws() %>% str_replace_all(" / ","_")
...
st_write(sample_points,"03_outputs\\01_sampling\\03_sample_point\\03_Returnees/total/all_points.kml",append = F,delete_dsn = T,delete_layer = T)
saveWidget(full_map, "03_outputs\\01_sampling\\03_sample_point\\03_Returnees/total/all_points.html",title = "Returnees sample point")
...
if(create_directory ==T){
  ...
  for (i in unique(sample_points$governorate)) {
    ...
    dir.create(path = paste0("03_outputs/01_sampling/03_sample_point/03_Returnees/",i))
  }
  ...
  for (i in unique(sample_points$district)) {
    ...
    dir.create(path = paste0("03_outputs/01_sampling/03_sample_point/03_Returnees/",gov,"/",i))
  }
  ...
  for (i in unique(sample_points$place_id)) {
    ...
    dir.create(path = paste0("03_outputs/01_sampling/03_sample_point/03_Returnees/",gov,"/",dis,"/",location))
  }
}
...
for(i in unique(sample_points$local_and_id)){
  ...
  st_write(sample_point_sub_dis, paste0("03_outputs/01_sampling/03_sample_point/03_Returnees/",
...
")) 
  ...
  map
  saveWidget(map,"map_re.html",title = i)
  
  file.copy(from = "map_re.html",to =  paste0("03_outputs/01_sampling/03_sample_point/03_Returnees/",
                                           governorate,"/",district,"/",sub_district,"/",
                                           str_replace_all(str_replace_all(paste0(sub_district),"-","_")," ","_"),
