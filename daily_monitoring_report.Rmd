...
text_cols <- (survey %>% filter(type == "text"))$name[!(survey %>% filter(type == "text"))$name%in% c( "enumerator_name" ,"contact_details_name", "referral_contact", "contact_details_telephone") ]
...
Cleaning_log_final <- Cleaning_log_final %>% 
  select(X_uuid,repeat_uuid,enumerator_name_id,ngo_label,loop,question,`label::English`,`label::Arabic`,old_value,issue,choice_list_from_kobo,pop_group,district,camp_name,contact_details_telephone) %>% 
